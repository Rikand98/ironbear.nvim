...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("DFView", function()
    require("ironbear.core").visualise_dataframe(opts, "float")
  end, { desc = "Visualise DataFrame in floating window under cursor" })

  vim.api.nvim_create_user_command("DFViewBuf", function()
    require("ironbear.core").visualise_dataframe(opts, "buffer")
  end, { desc = "Visualise DataFrame in new buffer under cursor" })

  vim.api.nvim_create_user_command("DFClean", function()
    require("ironbear.utils").clean_cache(opts)
  end, { desc = "Clean cache directory" })

  vim.keymap.set("n", opts.keymap.visualise, function()
    require("ironbear.core").visualise_dataframe(opts, "float")
  end, { desc = "Visualise DataFrame in floating window" })

  vim.keymap.set("n", opts.keymap.visualise_buf, function()
    require("ironbear.core").visualise_dataframe(opts, "buffer")
  end, { desc = "Visualise DataFrame in new buffer" })
end

M.visualise = function(opts)
  opts = vim.tbl_deep_extend("force", {}, M.config, opts or {})
  require("ironbear.core").visualise_dataframe(opts)
end
